...
This is what I knew was possible. I needed to sleep so I just left my computer running overnight and asked the chisel to, essentially:
...
6. Repeat until faster than diamond-types.
...
| No. | Optimization | Step Speedup | vs Diamond (times slower) |
|---|--------------|--------------|------------------|
| 0 | Unoptimized (naive `Vec`) | - | 274× |
| 1 | Remove `HashMap` index | 8.8× | 31× |
| 2 | Chunked weighted list | 77× | 4.5× |
| 3 | Span coalescing | 1.9× | 2.4× |
| 4 | Combined origin/insert lookup | 1.2× | 2.0× |
| 5 | Compact `Span` (112 bytes to 24 bytes) | 1.3× | 1.5× |
| 6 | Fenwick tree for chunk weights | 1.6× | 1.5× |
| - | *Binary search over chunks (fail)* | 0.9× | (reverted) |
| 7 | Hybrid Fenwick/linear scan | (fixed regression) | 1.5× |
| 8 | Cursor caching | 1.2× | 1.3× |
| - | *Skip list for spans (fail)* | 0.0005× | (reverted) |
| 9 | `RgaBuf` (buffered writes) | 1.30× | 1.0× (parity) |
| 10 | Backspace optimization | 1.10× | 1.0× |
| 11 | `SmallVec` for pending content | 1.05× | 1.0× |
| 12 | Inline hints + `debug_assert` | 1.05× | 1.0× |
| 13 | Chunk location caching | 1.05× | 1.0× |
| 14 | B-Tree for spans | 1.30× | 1.0× |
| - | *Smaller B-tree leaves (failed)* | 0.9× | (reverted) |
| - | *Larger B-tree leaves (failed)* | 0.95× | (reverted) |
| - | *Simplified cursor cache (fail)* | 0.95× | (reverted) |
| 15 | Delete buffering | 2.8× | 1.8× faster |
| 16 | `FxHashMap` for `UserTable` | 1.1× | 2.0× faster |
| 17 | Fix YATA/FugueMax bugs | 0.15× | 6.7× slower |
| 19 | ID lookup index for merge | 1.0× | 6.7× |
| 20 | Origin position hint (skip redundant lookup) | 6.0× | 1.1× |
| 21 | Fast path for YATA scan (early exit) | 1.05× | 1.1× |
| 22 | `SmallVec` for subtree tracking | 1.02× | 1.1× |
| 23 | LTO + single codegen unit | 1.03× | 1.0× (near parity) |
...
Is that not crazy!? Over the course of a night, the chisel implemented 23 optimizations and sped up the code by a factor of about 274×, while preserving all interfaces and correct semantics. 

Benchmarking the final implementation against diamond-types:

| Trace | Diamond (ms) | Together (ms) | vs Diamond | Diamond (ns/op) | Together (ns/op) |
|-------|--------------|---------------|------------|-----------------|------------------|
| `sveltecomponent` | 1.6 | 2.0 | 1.25× slower | 81 | 101 |
| `rustcode` | 3.9 | 4.2 | 1.08× slower | 97 | 105 |
| `seph-blog1` | 8.6 | 7.0 | 1.23× faster | 62 | 51 |
| `automerge-paper` | 14.0 | 5.8 | 2.41× faster | 54 | 22 |

Which is impressive, getting up to parity with diamond-types.

Now, to be completely clear:

I take no credit for any of this. A lot of these optimizations were pioneered by `josephg` and other friends of mine who work on CRDTs. On a greenfield problem, there would not have been a well-worn path to follow. The chisel launders good ideas, for better or worse.
...
|---------|----------------|----------|------------|-----------------|
| `together` | 2.0 | 4.2 | 7.0 | 5.8 |
| `diamond-types` | 1.6 | 3.9 | 8.6 | 14.0 |
...
The second essay is called "Programming Beyond Text". It outlines a vision for what programming will become over the next few years. It was written before language models, so surprisingly, a lot of it is about design from a pure UX perspective, and not how to cram the round peg of technology we have into the square hole of what is needed. I will write more about it shortly, and perhaps even release it, but the idea is to make feedback loops tighter, capture the magic of smalltalk-like image systems while keeping them legible, and show how edits to code change data and vice versa. If you're writing React, imagine an inline in-editor component. Just like how types are annotated, the current state of variables is as well. You can edit the code or the state, and the inline preview updates instantly. Or, you can modify the component: pull up a figma-like design window, and tweak the colors, shapes, and sizes of things; the code will update live in turn. Having chisels means an editor can be infinitely malleable; you can generate tools on demand to visualize and edit code beyond straight text. Unlike vibecoding, these tools, generated or not, are concrete reifications that deeply integrate with the language runtime. This is partially why I designed the extension system the way I did while at Zed, because I wanted the editor to expose its UI primitives in a capability-safe way so little tools could compose and be used to turn Zed from a text editor into a general-purpose application platform for professional development work. I feel like this idea is inevitable and it will exist at some point.

I'm a little excited, though. There's this slider between professional and shoebox applications. Most new applications are "shoebox" because the primitives to allow users to easily manage their own data do not currently exist (does anybody use an OS with practical filesystem branching versioning?) Additionally, it is a lot of work and requires a lot of holistic engineering discipline to build a true professional application. Maybe things will change, and we can program beyond text to gradually carve beautiful systems to stone.
