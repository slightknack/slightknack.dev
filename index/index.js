...
function display_scroll() {
    ...
    let target_height = Math.min(window.innerHeight * MOUNTAIN_OVERLAY, image_height * MOUNTAIN_OVERLAY);
    let height = image_height - target_height;

    first.style.right  = (SCROLL_WINDOW * SPEED % 1) * window.innerWidth + "px";
    first.style.bottom = (SCROLL_WINDOW * SPEED % 1) * image_height * MOUNTAIN_DROP - height + "px";
    first.style.visibility = "visible";

    second.style.right  = ((SCROLL_WINDOW * SPEED % 1) - 1) * window.innerWidth + "px";
    second.style.bottom = ((SCROLL_WINDOW * SPEED % 1) - 1) * image_height * MOUNTAIN_DROP - height+ "px";
...
function display_scroll_2() {
    ...
    let image_height = first.offsetHeight;
    let target_height = Math.min(window.innerHeight * CLOUD_OVERLAY, image_height * CLOUD_OVERLAY);
    let height = image_height - target_height;

    first.style.right  = (SCROLL_WINDOW * SPEED * 0.5 % 1) * window.innerWidth + "px";
    first.style.bottom = (SCROLL_WINDOW * SPEED * 0.5 % 1) * image_height * CLOUD_DROP - height + "px";
    first.style.visibility = "visible";

    second.style.right  = ((SCROLL_WINDOW * SPEED * 0.5 % 1) - 1) * window.innerWidth + "px";
    second.style.bottom = ((SCROLL_WINDOW * SPEED * 0.5 % 1) - 1) * image_height * CLOUD_DROP - height+ "px";
    second.style.visibility = "visible";
}
